...
describe('endpoint POST /user/register',()=>{
    test('responds with 201 status and user id of registered user',()=>{
        return request(app)
        .post('/user/register').send({firstName: 'bat', lastName: 'man', email:'batman@batmail.com', password: 'password123'})
        .expect(201)
        .then(({body})=>{
    ...
    test('responds with error if password is shorter than 8 characters', ()=>{
        return request(app)
        .post('/user/register').send({firstName:'bat', lastName: 'man', email:'batman@batmail.com', password: 'passwor'})
        .expect(400)
        .then(({body})=>{
    ...
    test('responds with error if invalid email is entered', ()=>{
        return request(app)
        .post('/user/register').send({firstName:'bat', lastName: 'man', email:'batman', password: 'passwor'})
        .expect(400)
        .then(({body})=>{
    ...
    test('responds with eror if first name is not entered', ()=>{
        return request(app)
        .post('/user/register').send({firstName:'', lastName: 'man', email:'batman', password: 'passwor'})
        .expect(400)
        .then(({body})=>{
    ...
    test('responds with error if last name is not entered',()=>{
        return request(app)
        .post('/user/register').send({firstName: 'bat', lastName: '', email:'', password: 'password123'})
        .expect(400)
        .then(({body})=>{
...
describe('endpoint POST /user/login',()=>{
    test('responds with 201 status and user id of registered user',()=>{
        return request(app)
        .post('/user/login').send({email:'batman@batmail.com', password: 'password123'})
        .expect(201)
        .then(({body})=>{
            expect(typeof body.user).toBe('string')
        })
    ...
    test('responds with 400 status and error message if password is incorrect',()=>{
        return request(app)
        .post('/user/login').send({email:'batman@batmail.com', password: 'password23'})
        .expect(400)
        .then(({body})=>{
    ...
    test('responds with 400 status and error message if email is not registered', ()=>{
        return request(app)
        .post('/user/login').send({email:'flash@flashmail.com', password: 'password23'})
        .expect(400)
        .then(({body})=>{
...
describe('endpoint POST /user/logout', ()=>{
    test('returns logged out response', ()=>{
        return request(app)
        .get('/user/logout')
        .expect(200)
        .then(({text})=>{
